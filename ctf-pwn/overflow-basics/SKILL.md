---
name: overflow-basics
description: "- [Stack Buffer Overflow](#stack-buffer-overflow)"
license: MIT
...
<!-- Original file: ctf-pwn/overflow-basics.md -->
...
`scanf("%d")` without sign check; negative input bypasses unsigned comparisons. See [advanced-exploits.md](../advanced-exploits/SKILL.md#signed-integer-bypass-negative-quantity) for full details.

## Canary-Aware Partial Overflow

Overflow `valid` flag between buffer and canary without touching the canary. Use `./` as no-op path padding for precise length control. See [advanced-exploits.md](../advanced-exploits/SKILL.md#canary-aware-partial-overflow) for full exploit chain.
...
**Pattern (Spreadsheet):** Overflow adjacent global variables via extra CSV delimiters to change filename pointer. See [advanced.md](../advanced/SKILL.md) for full exploit pattern.
