---
name: kernel-bypass
description: "- [KASLR and FGKASLR Bypass](#kaslr-and-fgkaslr-bypass)"
license: MIT
...
<!-- Original file: ctf-pwn/kernel-bypass.md -->
...
See [kernel.md - modprobe_path Overwrite](../kernel/SKILL.md#modprobe_path-overwrite) for the full technique, trigger sequence, and ROP payload.
...
See [kernel.md - core_pattern Overwrite](../kernel/SKILL.md#core_pattern-overwrite) for the full technique and how to find the `core_pattern` address.
...
- **Bypass:** Use kernel ROP (kROP) chains — all gadgets from kernel `.text`. See [kernel.md - Kernel ROP](../kernel/SKILL.md#kernel-rop-with-prepare_kernel_cred--commit_creds).
...
|-----------|--------|--------|
| SMEP | Executing userland pages from kernel | kROP (kernel ROP chain) — see [kernel.md](../kernel/SKILL.md#kernel-rop-with-prepare_kernel_cred--commit_creds) |
| SMAP | Accessing userland memory from kernel | kROP with heap-resident chain, `stac`/`clac` gadgets |
| No SMEP/SMAP | (nothing) | [ret2usr](../kernel/SKILL.md#ret2usr-no-smepsmap) — directly call userland privesc function |
