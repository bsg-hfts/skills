...
<!-- Original file: ctf-web/SKILL.md -->
...
- [recon-content-discovery.md](recon-content-discovery/SKILL.md) - Fast content discovery with FFUF, Feroxbuster, Gobuster, Dirsearch, and Wfuzz to uncover hidden routes, params, and vhosts
...
- [command-injection-automation.md](command-injection-automation/SKILL.md) - Commix-assisted command injection validation with conservative escalation and manual confirmation
...
- [server-side-2.md](server-side-2/SKILL.md) - XXE (basic, OOB, DOCX upload), XML injection via X-Forwarded-For, PHP variable variables, PHP uniqid predictable filename, sequential regex replacement bypass, command injection (newline, blocklist, sendmail CGI, multi-barcode, git CLI), GraphQL injection (introspection, batching, interpolation)
- [server-side-exec.md](server-side-exec/SKILL.md) - Direct code execution paths, upload-to-RCE, deserialization-adjacent execution, LaTeX injection, header and API abuses
- [server-side-exec-2.md](server-side-exec-2/SKILL.md) - More execution chains: SQLi fragmentation, path parser tricks, polyglot uploads, wrapper abuse, filename injection, BMP pixel webshell with filename truncation
- [server-side-deser.md](server-side-deser/SKILL.md) - Java/Python/PHP deserialization and race-condition playbooks, PHP SoapClient CRLF SSRF via deserialization
- [server-side-advanced.md](server-side-advanced/SKILL.md) - Advanced SSRF, traversal, archive, parser, framework, and modern app-server issues, Nginx alias traversal
- [server-side-advanced-2.md](server-side-advanced-2/SKILL.md) - Docker API SSRF, Castor/XML, Apache expression reads, parser discrepancies, Windows path tricks, rogue MySQL server file read
...
- [node-and-prototype.md](node-and-prototype/SKILL.md) - Prototype pollution, JS sandbox escape, Node.js attack chains
...
- SQL errors, odd filtering, or state-dependent DB behavior: start with [sql-injection.md](sql-injection/SKILL.md).
- Templating, file reads, SSRF, command execution, XML, or parser bugs: start with [server-side.md](server-side/SKILL.md) and [server-side-exec.md](server-side-exec/SKILL.md).
- XSS, CSP bypass, admin bot, client routing, DOM issues, or scriptless exfiltration: start with [client-side.md](client-side/SKILL.md).
- Session forgery, hidden admin routes, JWT, OAuth, SAML, or weak trust boundaries: start with [auth-and-access.md](auth-and-access/SKILL.md), [auth-jwt.md](auth-jwt/SKILL.md), and [auth-infra.md](auth-infra/SKILL.md).
- Node.js apps, prototype pollution, VM sandboxes, or SSRF into internal services: add [node-and-prototype.md](node-and-prototype/SKILL.md).
- Smart contract frontends or blockchain-integrated apps: add [web3.md](web3/SKILL.md).
...
Use [field-notes.md](field-notes/SKILL.md) once you have confirmed the challenge is truly web-heavy and you need the long exploit catalog.
