---
name: server-side-advanced-4
description: "- [WeasyPrint SSRF & File Read (CVE-2024-28184, Nullcon 2026)](#weasyprint-ssrf--file-read-cve-2024-28184-nullcon-2026)"
license: MIT
...
<!-- Original file: ctf-web/server-side-advanced-4.md -->
...
See also: [server-side-advanced.md](../server-side-advanced/SKILL.md) for Part 1 (ExifTool DjVu, Go rune/byte, ZIP symlink, path traversal bypasses, Nginx alias, Unicode homoglyph, Ruby Regexp.escape, /dev/fd, Flask/Werkzeug debug, XXE DTD filter bypass, %2f bypass). See also: [server-side-advanced-2.md](../server-side-advanced-2/SKILL.md) for Part 2. See also: [server-side-advanced-3.md](../server-side-advanced-3/SKILL.md) for Part 3.
...
**Key insight:** wget before 1.17.1 did not sanitize CRLF in the Host header. When SSRF reaches an internal SMTP service, CRLF injection enables sending arbitrary emails. Place the port at the END of the injected string to avoid "Bad port number" errors. This technique extends to any line-based protocol accessible via SSRF (FTP, Redis, memcached). See also [server-side.md](../server-side/SKILL.md#ssrf) for other SSRF techniques.
...
**References:** BSidesSF 2019 CTF — SVGMagic (PNGSVG), writeup 13711. See also the svglib variant in [server-side-2.md](../server-side-2/SKILL.md).
